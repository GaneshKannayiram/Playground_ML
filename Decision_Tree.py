...
# Set page layout
st.set_page_config(page_title="Interactive Decision Tree Playground", layout="wide")
...
# App title
st.markdown("<h1 style='text-align: center;'>🌳 Interactive Decision Tree Playground</h1>", unsafe_allow_html=True)
...
fig = px.scatter(X, x="Feature 1", y="Feature 2", color=y.astype(str), title="Decision Tree Decision Boundary")
...
plot_tree(model, filled=True, feature_names=X.columns, class_names=["0", "1"], ax=ax)
st.pyplot(fig_tree)
...
# Visualize Default Decision Tree Structure
st.header("Default Decision Tree Structure")
fig_default_tree, ax = plt.subplots(figsize=(12, 8))
plot_tree(default_model, filled=True, feature_names=X.columns, class_names=["0", "1"], ax=ax)
st.pyplot(fig_default_tree)
