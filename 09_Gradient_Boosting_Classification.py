...
# Set page layout
st.set_page_config(page_title="Interactive Gradient Boosting Playground", layout="wide")
...
# App title
st.markdown("<h1 style='text-align: center;'>🚀 Interactive Gradient Boosting Playground</h1>", unsafe_allow_html=True)
...
n_estimators = st.sidebar.slider("Number of Estimators (n_estimators)", 1, 100, 10, 1)
...
fig = px.scatter(X, x="Feature 1", y="Feature 2", color=y.astype(str), title="Gradient Boosting Decision Boundary")
...
st.header("Individual Trees in the Ensemble")
fig_trees, axes = plt.subplots(1, 3, figsize=(18, 6))
for i, ax in enumerate(axes):
    plot_tree(model.estimators_[i, 0], filled=True, feature_names=X.columns, class_names=["0", "1"], ax=ax)
st.pyplot(fig_trees)
